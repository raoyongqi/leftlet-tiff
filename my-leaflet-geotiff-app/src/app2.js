...
import 'leaflet.markercluster/dist/leaflet.markercluster'; // Ensure you have the correct package
...
const App = () => {
  ...
  const fetchAndRenderTIFF = async (filename, map) => {
    try {
      ...
      const georaster = await GeoRaster.fromArrayBuffer(response.data);

      const layer = GeoRaster({
        georaster,
        pixelToCanvas: (pixel) => pixel,
        opacity: 0.7,
        colorScale: 'viridis'
      });
